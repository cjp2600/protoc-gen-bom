...
func (e *RoleMongo) FillId(bom *bom.Bom) *bom.Bom {
	e.Id = primitive.NewObjectID() // create object id
	return e.WithBom(bom)
}
func (e *RoleMongo) InsertOne(bom *bom.Bom) (*RoleMongo, error) {
	e.Id = primitive.NewObjectID() // create object id
...
func (e *PermissionMongo) FillId(bom *bom.Bom) *bom.Bom {
	e.Id = primitive.NewObjectID() // create object id
	return e.WithBom(bom)
}
func (e *PermissionMongo) InsertOne(bom *bom.Bom) (*PermissionMongo, error) {
	e.Id = primitive.NewObjectID() // create object id
	res, err := e.WithBom(bom).InsertOne(e)
	if err != nil {
		return nil, err
...
func (e *UserMongo) FillId(bom *bom.Bom) *bom.Bom {
	e.Id = primitive.NewObjectID() // create object id
	return e.WithBom(bom)
}
func (e *UserMongo) InsertOne(bom *bom.Bom) (*UserMongo, error) {
	e.Id = primitive.NewObjectID() // create object id
	res, err := e.WithBom(bom).InsertOne(e)
	if err != nil {
		return nil, err
...
// create MongoDB Model from protobuf (ShortUserMongo)
type ShortUserMongo struct {
	Id        primitive.ObjectID `_id, omitempty`
	Active    bool
	FirstName string
	LastName  string
...
// The following are interfaces you can implement for special behavior during Mongo/PB conversions
// of type ShortUserMongo the arg will be the target, the caller the one being converted from

// ShortUserMongoBeforeToMongo called before default ToMongo code
type ShortUserMongoWithBeforeToMongo interface {
	BeforeToMongo(*ShortUserMongo) error
}

// ShortUserMongoAfterToMongo called after default ToMongo code
type ShortUserMongoWithAfterToMongo interface {
	AfterToMongo(*ShortUserMongo) error
}

// ShortUserMongoBeforeToPB called before default ToPB code
type ShortUserMongoWithBeforeToPB interface {
	BeforeToPB(*ShortUser) error
}

// ShortUserMongoAfterToPB called after default ToPB code
type ShortUserMongoWithAfterToPB interface {
	AfterToPB(*ShortUser) error
}

func (e *ShortUserMongo) ToPB() (*ShortUser, error) {
	var resp ShortUser
	var err error
	if prehook, ok := interface{}(e).(ShortUserMongoWithBeforeToPB); ok {
		if err = prehook.BeforeToPB(&resp); err != nil {
			return &resp, err
		}
	}
	...
	resp.FirstName = e.FirstName
	resp.LastName = e.LastName
	if posthook, ok := interface{}(e).(ShortUserMongoWithAfterToPB); ok {
		err = posthook.AfterToPB(&resp)
	}
	return &resp, err
}
...
// ToMongo runs the BeforeToMongo hook if present, converts the fields of this
// object to Mongo format, runs the AfterToMongo hook, then returns the Mongo object
func (e *ShortUser) ToMongo() (*ShortUserMongo, error) {
	var resp ShortUserMongo
	if prehook, ok := interface{}(e).(ShortUserMongoWithBeforeToMongo); ok {
		if err := prehook.BeforeToMongo(&resp); err != nil {
			return &resp, err
		}
	}
	...
	resp.FirstName = e.FirstName
	resp.LastName = e.LastName
	if posthook, ok := interface{}(e).(ShortUserMongoWithAfterToMongo); ok {
		if err := posthook.AfterToMongo(&resp); err != nil {
			return &resp, err
		}
	}
...
func (e *ShortUserMongo) WithBom(b *bom.Bom) *bom.Bom {
	return b.WithColl("user")
}
func (e *ShortUserMongo) FillId(bom *bom.Bom) *bom.Bom {
	e.Id = primitive.NewObjectID() // create object id
	return e.WithBom(bom)
}
...
func (e *TokenMongo) InsertOne(bom *bom.Bom) (*TokenMongo, error) {
...
func (e *ProviderUsersMongo) InsertOne(bom *bom.Bom) (*ProviderUsersMongo, error) {
	_, err := e.WithBom(bom).InsertOne(e)
	if err != nil {
		return nil, err
	}
